...
Assert::that($payload)
    ...
    ->keyExists('supportEmailAddress');

Assert::that($payload['supportEmailAddress'])
    ->nullOr()
    ->string('payload.supportEmailAddress must be string or null');

return $query
    ->withPayload($payload)
    ->withoutParam('customerId')
    ->withMethod('POST')
    ->withResource(\sprintf('/customer/customer/change-support-email-address'));
